...
```bash
azure_vote_start_variables=$(<aks_deployment.yml)
echo "${azure_vote_start_variables}" | sed -n '/azure-vote-start.yml/,/END/p' | kubectl apply -f
```
...
4. Apply Certificate Issuer YAML File
   ...
   ```bash
   cluster_issuer_variables=$(<aks_deployment.yml)
   echo "${cluster_issuer_variables//\$SSL_EMAIL_ADDRESS/$SSL_EMAIL_ADDRESS}" | sed -n '/cluster-issuer-prod.yml/,/END/p' | kubectl apply -f -
   ```

5. Update Voting App Application to use Cert-Manager to obtain an SSL Certificate.

   The full YAML file can be found in `azure-vote-nginx-ssl.yml`

  ```bash
  azure_vote_nginx_ssl_variables=$(<aks_deployment.yml)
  echo "${azure_vote_nginx_ssl_variables//\$FQDN/$FQDN}" | sed -n '/azure-vote-nginx-ssl.yml/,/END/p' | kubectl apply -f -
  ```
