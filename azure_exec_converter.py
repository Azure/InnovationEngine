...
def create_pr(filename):    
    ...
    with open('.github/workflows/auto-create-pr.yml', 'w') as f:
        f.write(f"""
    name: Create PR
    ...
    subprocess.run(f'git add .github/workflows/create_pr.yml', shell=True)
    subprocess.run(f'git commit -m "Add create PR GitHub action"', shell=True)
...
def create_pr(filename):    
    ...
    subprocess.run(f'git commit -m "Created AI Exec Doc titled {filename}"', shell=True)
