...
openai.api_key = '8198d83ccf2c478297dd9bd74bd0bf60' # os.getenv("OPENAI_API_KEY") # Create a Github instance using an access token or username and password
g = Github(login_or_token='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')
...
for dirpath, dirnames, filenames in os.walk('scenarios/ocd/AIDocs'):
    try:
        for filename in filenames:
            if 'exec' in filename or 'alpha' in filename:
                file_path = os.path.join(dirpath, filename)
                ie_test_command = f'./bin/ie execute {file_path}'
